...
class Vector {
  ...
  private:
    void resize() { d_capacity = d_capacity ? d_capacity+2 : 1; Data *newdata = (Data *)malloc(d_capacity*sizeof(Data)); memcpy(newdata, d_data, d_size * sizeof(Data)); free(d_data); d_data = newdata; };// Allocates two extra slots
};